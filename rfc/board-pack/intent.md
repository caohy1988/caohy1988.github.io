...
## Catalog live honesty update (2026-09-07, Claude Opus 5, `feat/board-pack-post42`)

Three site PRs merged after the leadership pass, and only one of them produced live evidence:

- **PR 40** (`e1cfd61`) — restricted-requester Slice A, **hermetic only**. Four stage-reachability cases and a requester broker; no live IAM, no live restricted-SA enforcement. Nothing on the page may credit it as a live result.
- **PR 41** (`7fd6539`) — Catalog-seeded chain Slice A, **hermetic only** (`catalog-mock`). Slice B live was explicitly not claimed.
- **PR 42** (`50ecaa8`) — Catalog **live** Slice B, the only new live evidence. **B1** (`live-20260907T153329Z-21d91539`): `python3 -m okf_bq_graph.chain --live --seed-mode catalog`. A fresh Dataplex read of the existing entry — one list page, 11 entries, exactly one match, then `entries.get(view=ALL)` for the `okf-context-runtime` aspect — resolved pin `pub_190192147fd7fd78` against the retained store; the head was observed and never followed; the clean pinned checkout (`31da799a`) compiled as the trusted projection. `CHAIN_CONNECTED`, three payloads `CONSISTENT`, `approved` RELEASED, `sql-substitution` REJECTED, `declaration-mismatch` MISMATCH at bind with the receipt never invoked; 23 jobs compared, `same_requester = SAME`, 0 unresolved. **B2** (`b2-20260907t162636z-cd45`): `python3 -m okf_bq_graph.catalog_live --live`, 222.97 s, nine chains plus cleanup on an isolated owned dataset and entry, **10/10 MET** — control, historical in-flight (head moved P1→P2 between pin resolution and payload read and P1 was still served exactly), historical fresh, `FAIL_STALE` on a withdrawn publication, `ASPECT_MISSING` at `seed` after the owned entry's `okf-context-runtime` aspect was removed (authored aspects intact — runtime pin metadata, not an authored rule), wrong publication pin, wrong seed pin, mixed-payload injection, recovery, cleanup, plus a `job-identity` gate reading all **170** jobs (48 lifecycle + 114 graph + 8 receipt) back under their own references. Astra APPROVE 0/0/0 at `ce428c2` after two fix rounds.

What this changes on the page, and what it must not imply:

- **Live catalog discovery is no longer untested.** The board pack and the full RFC said the chain "started from a fixture we chose rather than catalog discovery" and that "nothing has yet run from catalog discovery to a checked result". That was true at `157ec6d` and is stale now. R21/R22 in `spec.md` retire the fixture-seed qualifier for the 2026-09-07 runs and narrow the still-to-prove list.
- **The discovery claim keeps its own bound.** The returned pin is validated field by field against already-trusted configuration, so a catalog entry can confirm which publication to use but cannot name a different project or dataset; the catalog read is our own entry group, seeded earlier for the experiment, not a customer's.
- **One operator still holds both legs.** No second identity, no policy decision, no independent constrained attester. The PR 33 second-principal result stays isolated from the chain, and PR 40's restricted-requester work is hermetic — the page says offline tests exist and are not a live enforcement result.
- **GQL still blocked.** Relational/on-demand only: no embeddings, reservations, IAM changes, Enterprise window or graph DDL, every SELECT capped at 100 MiB billed. Benchmarks stay 0 of 9.
- **The mixed-payload refusals are client-side fault injection.** The live retrieval result was altered in our own client after BigQuery returned it; BigQuery did not return a torn publication. The page says so.
- **Not Alder, not customer data.** Acme invented gross margin; the receipt leg still binds to a synthetic fixture publication in the receipt example; the executed-SQL swap is that example's fixed case.
