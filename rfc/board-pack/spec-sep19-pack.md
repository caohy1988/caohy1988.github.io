...
| Fact data and its version | Named but unversioned in `evidence/chain/chain_live_restricted.json`: 7 tables in `test-project-0728-467323.okf_receipt_spike_20260905` (`fulfillment_cost`, `fx_daily_rates`, `order_lines`, `orders`, `payment_fees`, `products`, `shipment_cost`), reached through the receipt example's own publication `okf-receipt-spike/acme-retail-derived/gross-margin-period` at SDK pin `6719eb5`, a separate identity from the graph publication joined to it only by the declaration file's bytes. No snapshot, no as-of and no row version; the chain's own `as_of` is its run timestamp. Alder cohort data never selected | **UNSELECTED / INCOMPLETE** — prerequisite for the request-to-consumer comparison; blocks `sqlchain_forced_c1` / `sqlchain_forced_c5` |
...
Acceptance: the fact-version field is present and reads UNSELECTED rather than being folded into the corpus pin, and the corpus row no longer claims to be "what every experiment used" without qualification; the card names Haiyuan Cao as owner and states he has accepted nothing; the words "no Finance or data owner outside this project has been asked" appear; the historical 5.4 s is described as a partial sample from 28 of 100 attempts at C=1, not an agreement; "more demonstration evidence is not pilot validation" appears in the decision rule.
...
- A `facts` block records the fact-data gap: `state: UNSELECTED`, why it matters, what the retained chain does identify (read back from the chain record by test, not restated), what is missing, which cells it blocks and how to select one. `validate_plan` refuses a missing state, a `SELECTED` state with no recorded version or one that still blocks cells, an unexplained `UNSELECTED` state, or a `blocks` list naming a cell that does not exist. Both states render: the Markdown reports the plan's actual state, prints a `selected_version` (string or field by field) and drops the unselected-only sections, so a later selection cannot leave the JSON card and the Markdown card disagreeing.
...
Acceptance: every cell `INCOMPLETE`, every cost cell `UNMEASURED`, no prior observation filling a cell, the two consumer cells carrying `fact_version_blocked` with a stated reason, no surface saying failed attempts are "included in the denominator", and the committed artifacts byte-identical to their generator.
