...
- **Claims.** A grep for "nothing has (yet) run as one", "no part of it has run", "two separate/merged experiments" and "both experiments" over the five board-pack files matches only dated history and self-describing sentences: `intent.md` line 15 (the original 2026-09-06 UTC assessment, kept as the dated source) and the PR 35 addendum quoting the replaced wording, and `spec.md` R18 / its acceptance line. Visible prose of `index.html` and `STORY.md` (href values and Markdown link targets stripped) contains none of: HIGH, MODERATE, LOW, PARTIAL, BLOCKED, INCOMPLETE, INCONCLUSIVE, MEASURED, VERIFIED, UNVERIFIABLE, CHAIN_CONNECTED, MET, RELEASED, REFUSED, SDK, CLI, HMAC, JOINT, spike, PR numbers or commit hashes. R18 proximity was checked mechanically after the second fix pass: every sentence containing "chain" in the visible prose of `index.html` and `STORY.md` was printed with its neighbours and graded for the three qualifiers; all nine sites R18 lists carry fixture, requester and plain SQL within the chain sentence or the next (the first pass had left the two short notes without the requester, and the second pass had left the `#design-chain` opening, the `#design-evidence` bar sentence, both SVG `<desc>` openings and the STORY chain bullet without plain SQL until a later sentence). The only "chain" sentences without all three are the two bare cross-references to `#design-chain` and the "call it proven" bar text, which R18 now names as pointers.
...
- **Validation claim replaced** with what was actually checked: every sentence containing "chain" in the visible prose of `index.html` and `STORY.md` was printed with its neighbours and graded for the three qualifiers. The only sentences without all three are the two pointers, the bar text, mid-narrative sentences inside `#design-chain` and the STORY bullet after their qualified openings, the unrelated "a rule hidden in the middle of the chain" in `#design-access`, and the STORY sentence "not as one chain" about the separate-identity checks.
