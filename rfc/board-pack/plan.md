...
| "a withdrawn publication, a removed rule and two pins that did not exist each stopped the chain at the step that failed" | B2 `fail-stale-withdrawn` (`PUBLICATION_NOT_READY:WITHDRAWN` at `publication`), `missing-runtime-aspect` (`ASPECT_MISSING` at `seed`), `wrong-publication-pin`, `wrong-seed-pin` |
