...
### Catalog live honesty slice (Claude Opus 5, 2026-09-07 PT, `feat/board-pack-post42` from `50ecaa8`)

Wording and links only, both pages, one PR. No spike rerun, no cloud, no `styles.css` or SVG geometry change, no merge. Applied by exact-match replacement with an occurrence assertion on every old string: 15 replacement groups plus one block rewrite (the `#design-chain` body) in `rfc/board-pack/index.html`, 10 groups plus one block rewrite (the `#current-evidence` chain and what-remains paragraphs) in `rfc/index.html`, 6 groups plus one bullet rewrite in `STORY.md`, 6 copy-map rows plus an addendum in `spec.md`, an addendum in `intent.md`.

**What moved, and why.** PR 42 (`50ecaa8`) is the only one of the three merges since the leadership pass that produced live evidence. It supersedes exactly one claim: that the chain started from a fixture rather than live catalog discovery. `spec.md` R21 retires the fixture-seed qualifier for the 2026-09-07 runs and replaces it with the single operator, the plain-SQL engine and the Acme data boundary; R22 narrows the still-to-prove list to graph queries, a second identity and a policy decision, an independent constrained attester and real cohort data; R23 forbids describing PR 40's restricted-requester slice or PR 41's Catalog-seeded Slice A as live results, since both are hermetic.

**Claim map for the new sentences** (each checked against the artifacts at `50ecaa8`):

| New page wording | Artifact |
| --- | --- |
| "a chain began at a live catalog read"; "The read listed the entry group, matched one entry by name and fetched its runtime aspect" | README "Catalog-seeded chain, live": fresh Dataplex read of the existing entry, 1 list page, 11 entries, exactly 1 match, then `entries.get(view=ALL)` for the `okf-context-runtime` aspect |
| "a catalog entry can confirm which publication to use but cannot point the chain at a different project or dataset" | README Slice A validation paragraph: every returned field typed, non-empty and equal to trusted configuration; "Catalog values can never widen the allowlist or name a SQL destination" |
| "a substituted query was refused by the verifier; a mismatched declaration was refused before anything ran" | B1: `sql-substitution` → receipt `REJECTED` → `REFUSED`; `declaration-mismatch` → bind `MISMATCH`, receipt not invoked |
| "All 23 jobs that run submitted shared one requester" | B1: 21 journaled graph jobs + 2 receipt jobs = 23 compared, `same_requester = SAME`, `unresolved_jobs = 0` |
| "Nine more chains … same operator, same plain SQL … against an entry and a dataset we created for the test" | B2: isolated owned dataset + own Catalog entry, allowlist derived from `LifecycleConfig` before any Catalog read; nine full chains plus cleanup |
| "a head moved to a second version while a request was in flight and the request still served the version it had pinned, exactly" | B2 `historical-inflight` **MET**: resolution head P1, `SWITCHED`, head after P2, P1 served exactly — a retained serve, not a `FAIL_STALE` refusal |
| "a withdrawn publication, removed catalog metadata (the entry's runtime pin, with the authored entries checked and left intact — not an authored rule) and two pins that did not exist each stopped the chain at the step that failed, the metadata case at the very first step before any retrieval ran" | B2 `fail-stale-withdrawn` (`PUBLICATION_NOT_READY:WITHDRAWN` at `publication`), `missing-runtime-aspect` (the driver's `remove_runtime_aspect` deletes the owned entry's `okf-context-runtime` **aspect** only; `ASPECT_MISSING` at `seed`, zero cases run, both authored aspects verified intact — this is runtime pin metadata, **not** an authored knowledge rule, and not a denied or missing rule on the retrieval path: Astra PR 44 P2), `wrong-publication-pin`, `wrong-seed-pin` |
| "two payloads we tampered with ourselves, after BigQuery had returned them … BigQuery did not return a torn publication" | B2 `mixed-payload-injection`: live-backed **client** fault injection, raw live results retained with their pre-mutation SHA-256 under `injection/`; README states BigQuery did not return a torn publication |
| "the test entry and dataset were deleted and their absence read back, the original entry was unchanged" | B2 `cleanup` **MET**: originals `UNCHANGED` (identical entry SHA-256 and head before/after), owned entry and dataset deleted with absence readback |
| "all 170 jobs those runs submitted were read back one at a time: each had finished, under one identity" | B2 `job-identity` **MET** and `job_identity_followup.json`: 170 = 48 lifecycle + 114 graph + 8 receipt, each read under its own `(project, location, job id)`, all `DONE`, one principal |
| "the catalog runs' cleanup was rechecked against the cloud afterwards rather than taken from the run's own report" | README: `entries.list` back to 11 entries, owned dataset absent, `okf_graph_spike_20260905` untouched, plus the follow-up per-job audit |
| "offline restricted-requester work exists and is not a live enforcement result" | PR 40 is hermetic only (four stage-reachability cases, requester broker); PR 41 Slice A is `catalog-mock` |
| "cover more of that list … they still ran under one operator and on plain SQL in place of graph queries" | README "What these runs do not establish": not the connected bar, not RFC Phase A/2, no board-pack/STORY upgrade; single operator on both legs; relational/on-demand only |

Link pins: four new URLs at **E9** `50ecaa8976cb5fb633fb1c2b11b70eba3bbcd939` (`README.md#catalog-seeded-chain-live-2026-09-07-slice-b`, B1 `chain_live.json`, B2 `b2_summary.md`, B2 `job_identity_followup.json`). Every earlier `157ec6d` chain link is replaced; `bb88df4` graph links and `6719eb5` receipt links stay as dated history.

**Validation record (Claude Opus 5, 2026-09-07 PT).** Static checks served from the worktree with Playwright-managed Chromium, Firefox and WebKit; artifacts under `/tmp/okf-bp-post42/`.

- **Claims.** The R18/R21 paraphrase-plus-keyword audit (`/tmp/okf-bp-post42/audit_r21.py`) prints every sentence in the visible prose of `index.html` and `STORY.md` (href values and Markdown link targets stripped, SVG `<desc>` included) that contains "chain" or one of R18's paraphrases, with its neighbouring sentence, and grades it for the operator and engine qualifiers. All describing sites pass within the same or next sentence. **15 residual hits, each an enumerated carve-out:** the `.runtime-assessment` still-to-prove clause and the `.design-intro` pointer "The chain section below has the detail." (pointers/forward statements after a qualified opening); the `#design-access` sentence "A rule hidden in the middle of the chain stayed hidden" (about the graph's links, not the run); four mid-narrative sentences inside `#design-chain` and two inside the `STORY.md` chain bullet, all after their qualified openings; the bold lead-in "What these chains do not show." whose next sentence carries the operator; the two dated 2026-09-06 history sentences; the unchanged "call it proven" bar text; the "separate-identity checks … outside any chain" sentence; and the protected pilot ask "in one path". A banned-label scan of the same visible prose returns **0 hits** in `index.html` and 1 in `STORY.md` — the self-describing tone note that lists the banned categories.
- **Protected content.** Board pack `.hero`, `#later-questions`, the capacity line, `.punchline`, `.ask` and the "call it proven" bar sentence hash identically to `50ecaa8`. Full RFC: all `<pre>` blocks, all SVG geometry lines, `#decision`, the `#accept` checklist region and `#motivation` hash identically to `50ecaa8`; the only changed table rows are the Phase 2 and Phase 3 gate cells, and the only changed phase card is the Phase 2 "Added" cell. No new ID or class on either page; `styles.css` untouched.
- **Links.** The four new E9 URLs return HTTP 200; each path exists in the `50ecaa8` tree; the heading `## Catalog-seeded chain, live (2026-09-07, Slice B)` exists at that pin (line 342), giving the fragment. No dangling in-page anchor on either page in any engine.
- **Engines, board pack.** Chromium, Firefox and WebKit at 1280 and 320 px, disclosure closed and open: **932 closed-state words** in all three (910 before; ceiling amended to 940 in `spec.md`), 2,261 px tall closed at 1280 in Chromium (2,263 Firefox, 2,238 WebKit; open 5,106 / 5,129 / 5,051), document overflow 0 throughout, no duplicate IDs, no page or console errors. Enter opens and Space closes the disclosure. Print media with the disclosure closed renders the design body (`display: block`, 2,154 words) and the later-questions panel.
- **Engines, full RFC.** Chromium, Firefox and WebKit at 1280, 768 and 320 px: document overflow 0; `#repro-ask-later` measures 876/876 at 1280, 724/724 at 768 and 444 table inside a 276 px wrapper at 320; no duplicate IDs, no dangling anchors, no console errors. Both mastheads still read 3–4 minutes.
- **Diff.** `git diff --check` clean; changed tracked files are `rfc/index.html` and five board-pack files.

Not done: no cloud run, no experiment rerun, no merge, no change to §07 ask-later, `rfc/bq-vp/`, `rfc/full-demo/` or spike artifacts. Reviewers: Astra. Haiyuan retains the merge gate.

### Review fix pass — Astra PR 44 P2 (Claude Opus 5, 2026-09-07 PT, same branch)

Astra reviewed `74a34ff` (**REQUEST_CHANGES verdict, 0 P0 / 0 P1 / 1 P2**, posted as a comment review because the owner account cannot formally request changes on its own PR). The single finding was a documentation mismatch, not an evidence problem, and no experiment was rerun.
...
### Pass 2 honesty (Claude Fable 5.1, 2026-09-09 PT, `feat/board-pack-pass2-honesty` from `16d20ab`)

PR 57 merged Pass 2 of the ordinary-SQL baseline: campaign `sqlbase-20260909-065734-c50d0411` filled all four retrieval cells on-demand (card `RETRIEVAL_MEASURED`); consumer cells `NOT_IMPLEMENTED` + `FACTS_UNSELECTED`, cost cells `UNMEASURED`, GQL `OPTIONAL_LATER`, graph benchmark 0/9, thresholds `PROPOSED`. Edit list, rules R34–R40 and the validation record are in `plan-pass2-honesty.md` / `spec-pass2-honesty.md`; evidence pin E12 = `16d20ab`. Closed-state words 1,948 → 2,070 across three engines; protected regions unchanged; 58 offline tests pass; no cloud run, no merge.
