...
(function () {
  ...
  function renderSync() {
    ...
    return beatHead(4, "split", "Sync (CLI)", "Sync is an external CLI you run. Its direction is bundle → BigQuery commit → Catalog stamp. It has not run yet, and this beat does not pretend it has.",
      ...
      iamRow("operator, this capture (raincoatrun@gmail.com)", "project (Owner) · dataset okf_rfc_demo (OWNER)", "ran every bq, gcloud, curl and sample-script call in the default gcloud configuration; no service account created, nothing granted, nothing revoked", "live", "every job in bq_jobs_identity.json carries this user_email") +
      iamRow("bootstrap operator, Phase A (human Owner)", "project · dataset okf_rfc_demo · each SA", "time-boxed roleAdmin, projectIamAdmin, serviceAccountAdmin, catalogAdmin, aspectTypeOwner, dataset dataOwner; Token Creator per SA; all setup-only roles to be revoked after the tape", "rfc", "not done; no time-boxing, no SA, no binding yet") +
      iamRow("okf-setup (one-time)", "project · dataset", "catalogEditor + aspectTypeOwner for type/group creation; dataOwner + jobUser for DDL and seeds; then every role must be revoked (checks 6, 7)", "rfc", "not created; the operator (not okf-setup) ran the DDL") +
