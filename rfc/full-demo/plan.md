...
| Gate | Second run prints `no-op` and writes nothing. Kill the process between `BQ_COMMITTED` and stamp; rerun completes stamp without a new publication. Removing one concept file removes only the ledger-owned entry. `deployment_heads_history` shows two rows after a deliberate republish of an unchanged snapshot. All seven negative checks (nine API calls) in `spec.md` §1.3 return `PERMISSION_DENIED` on tape, including the two post-cleanup checks for `okf-setup`. |
