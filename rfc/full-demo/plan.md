...
| Bootstrap and identities (operator, human Owner) | The bootstrap operator holds the setup-only policy-owner roles of `spec.md` §1.3 (`roleAdmin`, `projectIamAdmin`, `serviceAccountAdmin`, `catalogAdmin`, `aspectTypeOwner`, dataset `dataOwner`) for the duration of setup, creates `okf-setup`, `okf-sync-writer-okf-rfc-demo`, `okf-runtime-reader` and the custom role `okfCatalogSearch`, and makes every binding on tape under its own identity in the order §1.3 gives: project-level type/group creation for setup only; `entryTypeUser` on `okf-bundle` and `aspectTypeUser` on `okf` and `okf-context-runtime` for the sync writer; `catalogEditor` on the one EntryGroup; **table-level** `dataEditor` on the nine runtime tables (no dataset grant, so `agent_events` is unreachable); `okfCatalogSearch` at project level for the reader; `serviceAccountTokenCreator` per SA. After setup: revoke every `okf-setup` role, run denial check 6, revoke its Token Creator, run check 7, delete the boundary EntryGroup, revoke the operator's own setup-only roles. |
| BigQuery DDL (as `okf-setup`, `sql/setup_runtime_tables.sql`) | New tables in `okf_rfc_demo`: `publications` (keyed by `publication_id`; `source` ∈ `sync`, `seeded_pre_phase_a`; `seeded_at`; owns no `context_ref` column), `deployments`, `deployment_heads`, `deployment_heads_history`, `concept_versions`, `snapshot_membership`, `relationship_assertions`, `context_ref_bindings` (append-only; unique on `context_ref`; Phase A on), `catalog_ownership`, `legacy_context_ref_bindings` (`context_ref`, `publication_id`, `origin`; may repeat a handle), `demo_evidence` (`source`, `context_ref`, `publication_id`, `note`). One view: `context_ref_resolution` = `context_ref_bindings` ∪ `legacy_context_ref_bindings` with a `binding_source` column. Table DDL, the view, and the seed `MERGE`s are all in the setup-owned file; the reader file `sql/attribution_two_key.sql` contains `SELECT`s only. Never touch `agent_events`. |
...
| Gate | Second run prints `no-op` and writes nothing. Kill the process between `BQ_COMMITTED` and stamp; rerun completes stamp without a new publication. Removing one concept file removes only the ledger-owned entry. `deployment_heads_history` shows two rows after a deliberate republish of an unchanged snapshot. All seven negative checks in `spec.md` §1.3 return `PERMISSION_DENIED` on tape, including the two post-cleanup denials for `okf-setup`. |
...
| Beat 4 | The Phase A asciinema tape, with `BQ_COMMITTED`, `CATALOG_STAMPED`, each positive-check `OK`, and each of the seven `PERMISSION_DENIED` calls as named marks. |
