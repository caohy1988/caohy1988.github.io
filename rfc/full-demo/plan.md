...
> Every item below that mentions a tape, a recording, `PERMISSION_DENIED`, `BQ_COMMITTED`, `CATALOG_STAMPED` or `no-op`
> is a future deliverable, and `/rfc/full-demo/` labels each one `RFC text only`. The only writes made for PR 16 were the
...
| Boundary probe (as `okf-setup`) | Phase A requirement, not yet run: create EntryGroup `okf-rfc-demo-boundary` and one disposable entry `boundary-probe` of type `okf-bundle`. `okf-setup` patches it (`OK`, positive check 1); the sync writer's identical patch is negative check 2 (`PERMISSION_DENIED`). Deleted in cleanup. No system entry (`@bigquery` etc.) is used for any check. |
...
| Pin-or-fail-stale | A `context_ref` bound to a non-head publication returns `FAIL_STALE`; junk returns `FAIL_CLOSED`. Both must be exercised in the tape. |
