...
| 4 | **Sync (CLI)** | `okf-context sync` as the sync-writer identity: validate → observe → snapshot → plan (adds 8, removes 0) → `BQ_COMMITTED` → heads advanced → new `context_ref` minted → `CATALOG_STAMPED` → status. Second run: `no-op`. Then the positive checks, boundary checks 1–5, and post-cleanup checks 6–7 (nine `PERMISSION_DENIED` calls in all). | asciinema tape of the Phase A CLI | Sync is external, idempotent, explicit-state, least-privilege. | **No.** Syncer not built; none of `BQ_COMMITTED`, `CATALOG_STAMPED`, `no-op` or `PERMISSION_DENIED` appears on the page. Page shows the algorithm and IAM contract as `RFC text only` plus the operator-run `setup.ts` / `push.ts` / DDL. |
