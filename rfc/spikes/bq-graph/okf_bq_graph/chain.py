...
def run_chain(engine: str, live: bool, sdk_root: str, out_dir: str, clients: Optional[dict] = None, projection: Optional[dict] = None,
              ...
              acme_root: Optional[str] = None, requester_mode: str = "operator", broker: Any = None) -> dict:
    ...
    out = {"chain": "okf_bq_graph.chain/0.5.0", "run_id": run_id, "mode": mode, "engine": engine,
    ...
    try:
        ...
        return _finish(out, out_dir, mode, redact, tag)
    ...
    # clients + requester
    if engine == "oracle" and projection is None and (clients is None or restricted):
        from .compile import compile_bundle
        projection = compile_bundle(acme_root or os.environ.get("OKF_ACME_ROOT", "/Users/haiyuancao/knowledge-catalog/okf/bundles/acme_retail"),
    ...
    def graph_leg(seed: str, path: str, cl: Optional[dict] = None, hidden: tuple = ()) -> tuple[dict, Optional[dict], Optional[dict]]:
    ...
    def restricted_case(case: str) -> dict:
        ...
        leg, comp, decl = graph_leg(spec["seed"], COMPUTATION_PATH, cc, hidden=spec["policy"]["hidden"])
        c.update(leg)
        r = c["retrieval"]
        if not r.get("reached"):
    ...
    elif any(s == "NOT_REACHED" for s in statuses) or ident_incomplete:
    ...
    if out["verdict"] != "CHAIN_CONNECTED":
        out["broken_at"] = next((c["case"] for c in out["cases"] if c["acceptance"]["status"] == "WRONG"),
                                next((c["case"] for c in out["cases"] if c["acceptance"]["status"] == "NOT_REACHED"), fallback_at))
    return _finish(out, out_dir, mode, redact, tag)


def _finish(out: dict, out_dir: str, mode: str, redact: Callable, tag: str = "") -> dict:
    """Publish the record: atomically to `chain_<mode>[_restricted].json` (last writer wins, but it references only its
    own run directory) and as a retained copy inside the run directory when one exists."""
...
def main(argv: Optional[list[str]] = None) -> int:
    ap = argparse.ArgumentParser(description="connected graph -> receipt chain (fixture seed; operator or restricted requester)")
    ...
    out = run_chain(engine=engine, live=live, sdk_root=a.sdk_root, out_dir=a.out, as_of=a.as_of, acme_root=a.acme_root, requester_mode=a.requester)
    ...
    print(f"provenance_ok={out.get('provenance', {}).get('ok')} {ident} requester={out.get('requester', {}).get('mode')} "
