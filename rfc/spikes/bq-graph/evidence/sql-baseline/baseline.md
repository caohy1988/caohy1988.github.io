...
**Cells this blocks.** None. Selecting a version clears `FACTS_UNSELECTED` only; the request-to-consumer cells stay INCOMPLETE because no sampled runner exists (NOT_IMPLEMENTED, which the cell table gives), and the live rows have not been verified against the selected digest (a full readback, not a count check, which no runner does yet).
