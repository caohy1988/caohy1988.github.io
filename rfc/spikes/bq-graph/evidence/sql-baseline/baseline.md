# Ordinary-SQL baseline — predeclared, campaigns retained, not measured

`sql-baseline/0.1.0` · declared 2026-09-08T01:00:00Z · **INCOMPLETE**

3 campaign(s) retained; 0 of 4 retrieval cells COMPLETE, 1 with any measured attempt. Every other cell is INCOMPLETE or UNMEASURED with its reason. No number below is invented: a cell shows only what its latest campaign's summary carries.
...
Campaign `sqlbase-20260909-064104-4266675c` stopped at the preflight (dry run, no job, nothing billed): **ON_DEMAND_OVERRIDE_DENIED**.

* Server: `400 POST https://bigquery.googleapis.com/bigquery/v2/projects/test-project-0728-467323/jobs?prettyPrint=false: Override to 'none' is not enabled. The option 'reservation_override_mode' is set to 'RESERVATION_OVERRIDE_MODE_UNSPECIFIED'. See https://cloud.google.com/bigquery/docs/default-configuration for configuration details. Location: US Job ID: 53820a40-a9e3-436b-954d-056dd8fffe16`
* Requires: `reservation_override_mode = ALLOW_ANY_OVERRIDE` on the project or organization (https://docs.cloud.google.com/bigquery/docs/default-configuration).
* Remedy: the project (or its organization) must set reservation_override_mode = 'ALLOW_ANY_OVERRIDE' (BigQuery ALTER PROJECT SET OPTIONS, permission bigquery.config.update); this driver does not change project settings
* Who: whoever holds bigquery.config.update on the project (or its organization); not this driver, not an agent
* Why it was not worked around: running the cells under the standing Enterprise assignment would be a different measurement (the plan's reservation line says so) and the routing guard would stop it as a violation; removing the assignment is out of scope for a baseline run
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| `sqlbase_forced_c1` | retrieval_ms | forced | 1 | 44 / 100 | **INCOMPLETE** | 139 | 185 | 0% | 0 MiB | — | `sqlbase-20260909-063150-d30baa0e` | BYTES_BUDGET_UNRESOLVED |
| `sqlbase_forced_c5` | retrieval_ms | forced | 5 | 0 / 100 | **NOT_RUN_PREFLIGHT** | — | — | — | — | — | `sqlbase-20260909-064104-4266675c` | ON_DEMAND_OVERRIDE_DENIED |
| `sqlbase_natural_c1` | retrieval_ms | natural | 1 | 0 / 100 | **NOT_RUN_PREFLIGHT** | — | — | — | — | — | `sqlbase-20260909-064104-4266675c` | ON_DEMAND_OVERRIDE_DENIED |
| `sqlbase_natural_c5` | retrieval_ms | natural | 5 | 0 / 100 | **NOT_RUN_PREFLIGHT** | — | — | — | — | — | `sqlbase-20260909-064104-4266675c` | ON_DEMAND_OVERRIDE_DENIED |
