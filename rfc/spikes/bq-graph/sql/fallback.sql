...
hop1 AS (SELECT c.node_id AS seed, c.status AS seed_status, e.dst_id AS n1, [e.edge_id] AS edge_ids FROM c JOIN e ON e.src_id = c.node_id),
hop2 AS (SELECT h.seed, h.seed_status, h.n1, e.dst_id AS n2, ARRAY_CONCAT(h.edge_ids, [e.edge_id]) AS edge_ids FROM hop1 h JOIN e ON e.src_id = h.n1),
cand AS (
