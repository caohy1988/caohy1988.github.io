...
| `okf_bq_graph/sql_baseline_run.py` | Driver for the four predeclared retrieval cells (Slice B Pass 1, 2026-09-08, hermetic): reads the sql-baseline plan rather than `fixtures/scale.json`, builds one `benchmark.measure` cell per retrieval cell with **only that cell's shape** as its query list, engine `fallback`, both caches off, on-demand with no reservation window (`cell_seconds` + a `TOTAL_TIME_BUDGET` deadline only), and a fresh `sqlbase-*` run_id checked against the retained GQL summary before any client exists. `--dry-run` prints cells / queries / budget and constructs no client; `--live` (Pass 2) runs foreground and retains `evidence/sql-baseline/run_<run_id>.json`. Consumer cells (`sqlchain_*`) are refused: `NOT_IMPLEMENTED` (no sampled runner) until the FS-1 hermetic-only runner landed on 2026-09-09 and `RUNNER_HERMETIC_ONLY` since, plus `FACTS_UNSELECTED` until the 2026-09-09 synthetic fact selection; customer (Alder) data stays unselected. |
| `okf_bq_graph/consumer_run.py` | Hermetic sampled request-to-consumer runner (FS-1, 2026-09-09): `--dry-run` runs the subprocess-free gates and launches nothing; `--hermetic` repeats the predeclared `f_current` question through the in-process oracle graph, bind, the SDK example's hermetic receipt runner, verification and the consumer decision (2 warmups + 20 measured per cell at the cell's concurrency, plus one `f_revenue` probe declared in advance to refuse at bind with the SDK never invoked), retaining every attempt under `evidence/consumer/`. Gates: SELECTED only, validity window, vendored-manifest drift, a row-based oracle (Python Decimal, not GoogleSQL) reproducing 400 / 515, SDK provenance, publication pin; `admit_live` (expiry, `FACTS_DRIFTED`, OK with the bound digest) exists as a pure function for FS-2 and has not been run live. **No `--live` flag, no BigQuery client, nothing filled: the consumer cells read `RUNNER_HERMETIC_ONLY`.** Docs: `rfc/board-pack/{intent,spec,plan}-sqlchain-consumer-fs1.md`. Pass 2 (2026-09-09) added a dry-run **preflight** of the on-demand override before any hold, and lowercase run_ids (gate labels are BigQuery job labels). **Four live campaigns are retained: three measured nothing (an illegal label, then the project denying `reservation = none` until Haiyuan set `reservation_override_mode = ALLOW_ANY_OVERRIDE`), the fourth (`sqlbase-20260909-065734-c50d0411`) completed all four cells on-demand** — 1,678 jobs verified, 28.1 GiB / $0.17 at list. Card: RETRIEVAL_MEASURED. |
...
* **Four retrieval cells** on the `fallback` engine — forced-seed and natural-question shapes, each at C=1 and at C=5,
  ...
  measured C=5 too).
* **Two request-to-consumer cells**, `RUNNER_HERMETIC_ONLY` (`NOT_IMPLEMENTED` until 2026-09-09). `retrieval_ms` and
  `request_to_consumer_ms` are separate cells and are never substituted for one another, and the 2026-09-06 23-second
  three-case chain pass is neither metric. `benchmark.measure` stops at retrieval; `chain.py` runs each case once;
  `consumer_run.py` is the sampled driver and has dry-run and hermetic modes only, so nothing has been sampled live.
...
and labels the campaign with a fresh `sqlbase-<yyyymmdd>-<hhmmss>-<hex8>` run_id (lowercase: it seeds BigQuery job labels) that is checked against the retained GQL summary
...
* `sqlbase-20260909T062840Z-a3fc21f5` (18 s): every insert was rejected 400 — the run_id's uppercase `T`/`Z` made the
  per-cell gate label `<run_id>_<cell>`, which is the `window` label on every job, an illegal BigQuery label value.
  Each rejected insert kept its 1 GiB hold as liability (a client exception after the id was journaled is not proof of
  non-submission), so 64 rejections consumed the whole ceiling: `sqlbase_forced_c1` INCOMPLETE n=44 (all errors), the
  other three NOT_RUN_BUDGET, `BYTES_BUDGET_UNRESOLVED`, **0 bytes billed, $0**. Fixed offline: run_ids are lowercase
  and `assert_label_is_legal` refuses an illegal label before any client exists (`--dry-run --run-id` with the old id
  prints INVALID).
* `sqlbase-20260909-063150-d30baa0e` (19 s): same shape of failure for a different reason — BigQuery rejected the
  job-level override: `Override to 'none' is not enabled. The option 'reservation_override_mode' is set to
  'RESERVATION_OVERRIDE_MODE_UNSPECIFIED'`. Again 64 liabilities, 0 bytes, $0. The REST contract says so
  (`Job.configuration.reservation`: forcing on-demand "requires the project or organization to have
  reservation_override_mode set to ALLOW_ANY_OVERRIDE"); the project sits at the default, which allows moving a job to
  another reservation but not to on-demand. Astra's PR 55 re-review had noted that requirement and that the cloud
  configuration had not been read.
* `sqlbase-20260909-064104-4266675c` (0.9 s): the driver now runs one **dry-run preflight** of `SELECT 1` with the
  override before any hold — a dry run creates no job and bills nothing but is validated against the project
  configuration exactly like a paid insert. It was DENIED, every cell is `NOT_RUN_PREFLIGHT / ON_DEMAND_OVERRIDE_DENIED`,
...
`NOT_IMPLEMENTED + FACTS_UNSELECTED` until the fact-data selection below, `NOT_IMPLEMENTED` after it and `RUNNER_HERMETIC_ONLY` since the hermetic-only runner (below), the cost cells
...
read `NOT_IMPLEMENTED + FACTS_UNSELECTED` at the time, `NOT_IMPLEMENTED` after the selection and `RUNNER_HERMETIC_ONLY` since the hermetic-only runner, and this driver still refuses to run them.
...
as its own block. Selecting clears `FACTS_UNSELECTED` only: both consumer cells stayed `INCOMPLETE / NOT_IMPLEMENTED`
(`RUNNER_HERMETIC_ONLY` since the runner below), the cost cells `UNMEASURED`, every threshold PROPOSED. Docs:
`rfc/board-pack/{intent,spec,plan}-sqlchain-fact-select.md`.

**Hermetic-only consumer runner (FS-1, 2026-09-09).** `okf_bq_graph.consumer_run` is the sampled request-to-consumer
driver in its dry-run and hermetic modes; there is no `--live` flag and no BigQuery client in the module. A hermetic
campaign repeats the predeclared `f_current` question (the fixed January `approved` case) through the in-process oracle
graph, bind, the SDK example's hermetic receipt runner, verification and the consumer decision, and retains every
attempt with its retrieval time, bind outcome, the emulation's job identity, verdict, decision, SDK elapsed time,
the selected content digest and the evaluation date; each cell also runs one `f_revenue` probe declared in advance to
refuse at bind (`file_sha256` and `sql_text` fail, the SDK is never invoked). Before any attempt: SELECTED only, the
validity window (`VALIDITY_WINDOW` before 2026-03-12), vendored-manifest drift, a row-based oracle over the vendored
manifest that must reproduce 400 (January) and 515 (January–February) — Python Decimal, not GoogleSQL — SDK provenance
and the publication pin. `admit_live` (expiry on or after the recorded date, `FACTS_DRIFTED` when a readback does not
hash to the content digest, `OK` with the digest to bind) is the live admission FS-2 must call; here it is exercised by
tests only. One campaign is retained under `evidence/consumer/` (both cells at 2 + 20 plus a probe each, C=1 then
C=5, `RUNNER_HERMETIC_OK`); the card lists it by id with attempt counts and decisions and prints none of its times,
because they are orchestration overhead of an emulation on one laptop, not `request_to_consumer_ms`. The consumer
cells therefore read `RUNNER_HERMETIC_ONLY`, still a refusal; a consumer sampling budget (8 GiB / $0.05 inside the
64 GiB / $0.50 ceilings) is declared in the plan and not spent. Docs: `rfc/board-pack/{intent,spec,plan}-sqlchain-consumer-fs1.md`.
...
python3 -m okf_bq_graph.sql_baseline                         # offline: regenerate the SQL baseline card from the plan + retained campaign records
python3 -m okf_bq_graph.sql_baseline_run --dry-run           # offline: print the baseline campaign (cells, queries, budget); no client
python3 -m okf_bq_graph.sql_baseline_run --live [--cells sqlbase_forced_c1 ...]   # Pass 2 (paid, on-demand, foreground): dry-run preflight of the override, then the cells
```
