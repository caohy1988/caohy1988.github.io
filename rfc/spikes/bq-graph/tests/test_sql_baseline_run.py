...
def test_budget_comes_from_the_plan_and_has_no_window(plan, cases):
    ...
    assert campaign["reservation"] is None and campaign["edition"] == "on-demand"
...
def test_driver_never_imports_the_reservation_layer():
    ...
    assert not imported & {"reservation", "lifecycle", "run", "open_window", "close_window", "WindowJobs", "WindowClient"}
    assert not hasattr(run, "open_window") and not hasattr(run, "WindowJobs")
...
def test_dry_run_opens_no_client_and_prints_the_campaign(no_client, tmp_path):
    ...
    assert "engine fallback, on-demand, reservation none" in text
...
def test_live_hands_measure_one_config_and_retains_the_record(plan, cases, scratch, no_client):
    ...
    assert saved["cells"] == record["cells"] and saved["edition"] == "on-demand" and saved["reservation"] is None
...
def test_live_refuses_a_window(plan, cases, scratch):
    campaign = run.build_campaign(plan, cases, run_id="sqlbase-20260919T000000Z-deadbeef")
    campaign["reservation"] = "US.okf-demo-enterprise"
