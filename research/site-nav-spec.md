...
| Lab | `/research/harness-lab/` | — |
| Builds ▾ | folder | RFC `/rfc/` · Detailed RFC `/rfc/detailed-rfc/` · EvalBench `/evalbench/` · MCP Apps matrix `/research/builds/mcp-apps-feature-matrix/` |
...
- Pages in scope (15+): `research/index.html`, the research section `*/index.html` pages (including harness-lab), `research/builds/mcp-apps-feature-matrix/index.html` (generated by `tools/build_mcp_apps_matrix.mjs`, which imports the nav renderer), `rfc/index.html`, `rfc/detailed-rfc/index.html`, `rfc/demo/index.html`, `rfc/full-demo/index.html`, `evalbench/index.html`. Per-page topbar/brand/nav CSS is removed; the board pack masthead loses its duplicate brand link and keeps its audited kicker line.
...
1. `node tools/site_nav.mjs --check` → all 14 pages in sync.
